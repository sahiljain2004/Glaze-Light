...
const SaleScreen = () => {
    ...
    const saleData = route?.params?.saleData || null;
    const isEditMode = route?.params?.isEditMode === true;
    const isFormEditable = !isEditMode || isEditing;

    const [description, setDescription] = useState(saleData?.description || "");
    const [receivedAmount, setReceivedAmount] = useState(saleData?.receivedAmount?.toString() || "");
    const [paymentMethod, setPaymentMethod] = useState(saleData?.paymentMethod || "Cash");
    const [isReceived, setIsReceived] = useState(saleData?.isReceived || false);
    const [customerName, setCustomerName] = useState(saleData?.customerName || saleData?.name || "");
    const [phoneNumber, setPhoneNumber] = useState(saleData?.phoneNumber || "");
    const [address, setAddress] = useState(saleData?.address || "");
    const [saleType, setSaleType] = useState(saleData?.saleType || "Credit");
    const [items, setItems] = useState(saleData?.items || []);
    ...
    useEffect(() => {
        ...
        setItems((previousItems) => {
            const itemAlreadyExists = previousItems.some((item) => item.id === newItem.id);
            if (itemAlreadyExists) return previousItems;
    ...
    const dueAmount = Math.max(totalAmount - received, 0);
    ...
    const handleUpdateSale = () => {
        const updatedSale = {
            ...
            balance: Number(dueAmount).toFixed(2),
    ...
    const handleEditSale = async () => {
        ...
        try {
            const { data } = await api.put(`/api/transactions/${saleData.id}`, updatedData);
        ...
        Alert.alert(
            ...
            [
                { text: "Cancel", style: "cancel" },
                {
                    ...
                    onPress: async () => {
                        try {
                            const { data } = await api.delete(`/api/transactions/${saleData.id}`);
        ...
        <View style={styles.container}>
            <View style={styles.header}>
                ...
                <Text style={styles.headerTitle}>
                    {isEditMode ? "Sale Details" : "Sale"}
                </Text>

                <View style={styles.saleTypeContainer}>
                    <TouchableOpacity
                        style={[styles.typeButton, saleType === "Credit" && styles.activeType]}
                        onPress={() => setSaleType("Credit")}
                    >
                        <Text style={[styles.typeText, saleType === "Credit" && styles.activeTypeText]}>Credit</Text>
                    </TouchableOpacity>

                    <TouchableOpacity
                        style={[styles.typeButton, saleType === "Cash" && styles.activeType]}
                        onPress={() => setSaleType("Cash")}
                    >
                        <Text style={[styles.typeText, saleType === "Cash" && styles.activeTypeText]}>Cash</Text>
                    </TouchableOpacity>
                </View>
            </View>
            ...
            >
                <View style={styles.invoiceRow}>
                    <View>
                        ...
                        <Text style={styles.invoiceText}>{saleData?.invoiceNumber || "7155"}</Text>
                    </View>
                    <View>
                        <Text style={styles.smallLabel}>Date</Text>
                        <Text style={styles.invoiceText}>
                            {saleData?.date || new Date().toLocaleDateString("en-IN")}
                        </Text>
                    </View>
                </View>
                ...
                <BilledItemsCard
                    items={items}
                    setItems={setItems}
                    onAddItem={isEditing || !isEditMode}
                    editable={isFormEditable}
                />
